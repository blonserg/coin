        ...
        <div v-for="item in userProfileProjects" :key="item.id" class="profile-refitem d-flex align-center justify-space-between">
          ...
          <div v-if="item.link" class="header-refs header-refs--mycom d-flex align-center">
            <v-text-field
              ...
              :disabled="editUser"
            />
            <div @click="postUserProjectLinks(item)">Зберегти</div>
...
export default {
  ...
  methods: {
    ...
    async postUserProjectLinks (item) {
      const projectParamsKey = "projects_links[" + item.id + "]";
      const projectParams = {};
      projectParams[projectParamsKey] = item.link;
      const params = {
